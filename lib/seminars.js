...
// 키워드 → 매칭 동의어. 제목에 정확한 글자가 없어도 관련 분야를 잡아낸다.
// (장소명과 겹치는 도서관·박물관, 정치/미디어로 번지는 방송·미디어, 군사와 겹치는 전시 등은 제외)
export const KEYWORD_SYNONYMS = {
  "문화": ["문화", "콘텐츠", "문화재", "한류", "출판", "영화", "웹툰", "애니메이션"],
  "예술": ["예술", "미술", "공연", "음악", "디자인", "공예", "연극", "무용"],
  "AI": ["ai", "인공지능"],
  "산업정책": ["산업정책", "산업 정책", "제조업", "산업전략"],
  "저출산": ["저출산", "저출생", "인구위기", "인구절벽"],
  "지역균형": ["지역균형", "균형발전", "지방소멸", "지역소멸"],
};
function synonymsFor(keyword) {
  return (KEYWORD_SYNONYMS[keyword] || [keyword]).map((s) => s.toLowerCase());
}

export function matchedKeywords(event, activeKeywords, keywords = KEYWORDS) {
  const active = activeKeywords || new Set(keywords);
  // 일정 API의 설명문(text)은 주최·일시·"장소"가 반복돼 장소명이 오탐을 부르므로 매칭에서 제외
  const haystack = `${event.title} ${event.host} ${event.cohost} ${event.committee}`.toLowerCase();
  return keywords.filter(
    (keyword) => active.has(keyword) && synonymsFor(keyword).some((s) => haystack.includes(s))
  );
}
